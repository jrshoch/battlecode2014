...
public class RobotPlayer {
  ...
  private static int simulateOutcome(RobotInfo[] attackableRobotInfos,
    ...
    while (totalEnemies > 0 && totalAllies > 0) {
      int alliedKills = simulateHits(totalEnemies, totalAllies, alliedBuckets);
      totalAllies -= alliedKills;
      int enemyKills = simulateHits(totalAllies, totalEnemies, enemyBuckets);
      totalEnemies -= enemyKills;
      if (enemyKills - alliedKills != 0) {
        return enemyKills - alliedKills;
      }
    }
  ...
  private static int simulateHits(int totalTeamA, int totalTeamB, int[] bucketsTeamB) {
    ...
    while (hitsTeamA > 0) {
      if (bucketsTeamB[bucketIndex] > 0) {
        ...
        if (hitsTeamA <= bucketIndex + 1) {
          assert bucketsTeamB[bucketIndex] == 1;
          bucketsTeamB[bucketIndex] -= 1;
          bucketsTeamB[bucketIndex - hitsTeamA] += 1;
          assert bucketsTeamB[bucketIndex - hitsTeamA] == 1;
          hitsTeamA = 0;
        }
        bucketIndex++;
      }
